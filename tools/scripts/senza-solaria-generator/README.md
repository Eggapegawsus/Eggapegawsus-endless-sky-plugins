...
## How to Use
 ...
 - The script will then give you an option to output everything into either a single folder (I would advise "yes" unless you are testing something), or copy the original folder structure that it found the files in
 - It will then give you the option to have the base ship model listed along side the variant in the "Ship Variants" file (I would advise "no" unless you are testing something.)
 - The script will then parse through the given directory, and find all ships and outfits, making a mission for each item found to be included in Senza Solaria, as well as making a file containing the names of all found outfits, ship models, and variants.
## More Info
...
**Additional Information**
 ...
 - Due to the way the script is made, for the "unlock-all" files to fully generate with a complete list of found outfits or ships in a chosen directory, **_you will need to do a completely fresh scan every time_** without any pre-existing Senza Solaria Generator output files in the output directory. As said before, **ALWAYS be sure to backup your files before attempting to update them.**
<br>
